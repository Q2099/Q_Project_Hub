document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateSummary() {
    ...
    if (rooms >= 2) guestSummary += `, Room 2: ${guests2} guest${guests2 > 1 ? "s" : ""}`;
    if (rooms === 3) guestSummary += `, Room 3: ${guests3} guest${guests3 > 1 ? "s" : ""}`;

    const summaryText = `${air} | Class: ${cls} | ${rooms} Room${rooms > 1 ? "s" : ""} | ${guestSummary}`;
    //const summaryText = `${from} → ${dest} | ${dep} → ${ret} | ${air} | Class: ${cls} | ${rooms} Room${rooms > 1 ? "s" : ""} | ${guestSummary}`;
  ...
  const outboundDateEl = document.getElementById("cc_sb3_sb3_flighthotel_sb3_flighthotel_outboundDate");
  const returnDateEl = document.getElementById("cc_sb3_sb3_flighthotel_sb3_hotel_numNights");

  if (outboundDateEl) {
    outboundDateEl.addEventListener("input", updateSummary);
    outboundDateEl.addEventListener("change", updateSummary);
  }
  if (returnDateEl) {
    returnDateEl.addEventListener("input", updateSummary);
    returnDateEl.addEventListener("change", updateSummary);
  }

  // --- Attach other inputs/selects dynamically ---
  document.querySelectorAll("input, select").forEach(el => {
    el.addEventListener("input", updateSummary);
    el.addEventListener("change", updateSummary);
  });

  // --- Autosuggest click triggers ---
  document.addEventListener("click", e => {
    if (e.target.classList.contains("suggestion-item")) {
      const inputEl = e.target.closest(".fbx-25")?.querySelector("input");
      if (inputEl) {
        inputEl.value = e.target.dataset.value || inputEl.value;
        updateSummary();
      }
    }
  });
